...
void populate_root_servers_tab(struct w *widgets)
{
    ...
    long file_size=0;
    char *line, *buffy;
    gchar *filepath, *utf8=NULL;
    GtkTextBuffer *text_buffer;

    text_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(widgets->rs_textview));

    filepath = g_strdup_printf("%s/etc/root.hints", CHROOT_PATH_BUF);
    if((fp=fopen(filepath, "r"))==NULL)
    {
      /* If theres a popup here itll also pop up when chroots are changed
         and if the chroot is missing. We dont want that to happen. */
        g_free(filepath);
        return;
    }
    fseek(fp, 0, SEEK_END);
